...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level - 1]:
            for literal in a_node.effnodes:
                a_node.children.add(literal)
                literal.parents.add(a_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for x in node_a1.action.effect_add:
            if x in node_a2.action.effect_rem:
                return True
        for y in node_a1.action.effect_rem:
            if y in node_a2.action.effect_add:
                return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for x in node_a1.action.effect_rem:
            if x in node_a2.action.precond_pos:
                return True
        for y in node_a2.action.effect_add:
            if y in node_a1.action.precond_neg:
                return True
        for y in node_a2.action.effect_rem:
            if y in node_a1.action.precond_pos:
                return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for l in range(len(self.s_levels)):
            if not goals:
                break
            for n in self.s_levels[l]:
                if n.is_pos:
